...
public class BufferDao {
	/*
	 * 
	 * 보조스트림 : 기반스트림만으로 부족했던 성능을 보다 향상 시켜주는 스트림 기반스트림에서 제공하지 않는 추가적인 메소드를 제공 데이터 전송
	 * 속도를 향상 시킴 + a
	 * 
	...
	public void fileRead() {
		...
		try(BufferedReader br = new BufferedReader(new FileReader("c_buffer.txt"))){
			...
			*/
			...
			String value = null;
			while( (value=br.readLine()) != null) { // null과 비교시에는 equals 안써도 됨
				System.out.println(value);
			}
			
			
			
		} catch (FileNotFoundException e) { // 자식 => IOException 보다 위에 있어야 함
			e.printStackTrace();
