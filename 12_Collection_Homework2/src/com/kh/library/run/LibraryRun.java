...
public class LibraryRun {
	...
	public static void main(String[] args) {// 소장책 DB
		...
		printBooks(bList); // bs에 들어감
	}

	public static void printBooks(ArrayList<Book> bs) {

		BookController bc = new BookController();
		// bc.addBook(bs); // 위에서 만든 bList를 bs로 넘겨서 추가
		// addBook 은 북 타입
		// bs.get(index) => 북객체
		// bc.addBook(bs.get(0));

		// bList 는 위의 메소드에서만 사용가능

		for (Book b : bs) {
			bc.addBook(b); // b = bs.get(index)
		...
		while (flag == true) { // 무한반복
			...
			case 1:
				...
				if (search == 1) {
					...
					flag = true; // true면 while문 타라는 것 집에서 찾아보기
				}
				break;
			...
			case 2:
				...
				String bNo = sc.next(); // 핵심

				// nextxxx 함수 뒤에 nextLine(); 써줘야함
				// sc.nextLine();
			...
			case 4: // 전체 책 합계, 평균
				System.out.println("전체 책 가격 합계 : " + bc.getTotalPrice() + "원 입니다");
				System.out.println("전체 책 가격 평균 : " + bc.getAvgPrice() + "원 입니다");
				break;
